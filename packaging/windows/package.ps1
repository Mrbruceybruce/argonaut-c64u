...
$setup = Join-Path $assets 'Argonaut-0.1.1-Windows-x64-Setup.exe'
...
Compress-Archive -Path "$bundle\*" -DestinationPath "$assets\Argonaut-0.1.1-Windows-x64-Portable.zip"
git archive --format=zip --prefix=argonaut-0.1.1/ -o "$assets\argonaut-0.1.1-source.zip" HEAD
