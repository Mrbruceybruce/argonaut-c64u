...
$setup = Join-Path $assets 'Argonaut-1.5-Windows-x64-Setup.exe'
...
Compress-Archive -Path "$bundle\*" -DestinationPath "$assets\Argonaut-1.5-Windows-x64-Portable.zip"
git archive --format=zip --prefix=argonaut-1.5/ -o "$assets\argonaut-1.5-source.zip" HEAD
