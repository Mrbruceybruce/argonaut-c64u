...
TASKS = (
    ...
    'Open a D64 directory. Confirm Extract, Add, Rename, and Remove use labeled icons; Discard changes and Save image as… are separate at the right; the title does not show an I-beam; and unsaved staged changes still require confirmation before closing.',
    'Choose Add file… and select at least two small host files at once. Confirm each receives a C64 filename prompt, Return accepts Add file, both changes are staged, and Save image as… opens in the visible local folder. Reopen the result and verify both files.',
